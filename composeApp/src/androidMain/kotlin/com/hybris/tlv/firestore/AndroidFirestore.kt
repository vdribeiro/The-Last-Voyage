...
internal class AndroidFirestore: Firestore {
    ...
    override suspend fun setCollection(collection: String, documents: List<Map<String, Any>>): Flow<FirestoreWriteResult> = flow {
        ...
        for (i in 0 until numBatches) {
            ...
            for (document in batchDocuments) {
                val id = document.getString(key = "id")
                if (id == null) {
                    emit(value = FirestoreWriteResult.Error(error = "Document id is null"))
                    return@flow
                }
    ...
    override suspend fun setDocument(collection: String, document: Map<String, Any>): Boolean {
        return firestore
            .collection(collection)
            .document(document.getString(key = "id") ?: return false)
            .set(document.toMutableMap().apply { remove(key = "id") })
            .tryAwait()
    }
    ...
    override suspend fun removeCollection(collection: String): Flow<FirestoreWriteResult> = flow {
        val totalDocuments = getCollectionCount(collection = collection)
        if (totalDocuments == null) {
            emit(value = FirestoreWriteResult.Error(error = "Error getting documents"))
            return@flow
        }

        do {
            val batch = firestore.batch()
            val documentsInBatch = firestore
                .collection(collection)
                .limit(BATCH_SIZE.toLong())
                .get()
                .tryAwaitWithResult()?.documents
            ...
            batch.commit().tryAwait()
            emit(value = FirestoreWriteResult.PartialSuccess(documents = documentsInBatch.mapNotNull {
                it.data?.toMutableMap()?.apply { put(key = "id", value = it.id) }
