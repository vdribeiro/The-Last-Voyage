...
internal fun HabitabilityContent(store: Store<MainMenuAction, MainMenuState>) {
    ...
    LazyColumn(
        modifier = Modifier.thenIf(
            tag = MAIN_MENU_SCREEN_HABITABILITY_CONTENT,
        ...
        items(items = formula, key = { it.id }) { property ->
            SimpleCard(
                modifier = Modifier.thenIf(tag = MAIN_MENU_SCREEN_HABITABILITY_CONTENT_SIMPLE),
                name = property.id,
        ...
        item {
            Text(
                modifier = Modifier.thenIf(
                    tag = MAIN_MENU_SCREEN_HABITABILITY_CONTENT_FORMULA,
                    onClick = { uriHandler.openUri(uri = storeState.formula) }
                ),
