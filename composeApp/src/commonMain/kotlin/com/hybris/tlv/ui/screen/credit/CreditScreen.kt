...
import com.hybris.tlv.ui.theme.component.grid.CreditItem
...
internal fun CreditScreen(store: Store<CreditState, Unit>) {
    ...
    val creators = creditsMap[CreditType.CREATOR].orEmpty().map { CreditItem(id = it.id, link = it.link) }
    val sources = creditsMap[CreditType.SOURCE].orEmpty().map { CreditItem(id = it.id, link = it.link) }
    val musics = creditsMap[CreditType.MUSIC].orEmpty().map { CreditItem(id = it.id, link = it.link) }
    val supporters = creditsMap[CreditType.SUPPORTER].orEmpty().map { CreditItem(id = it.id, link = it.link) }
