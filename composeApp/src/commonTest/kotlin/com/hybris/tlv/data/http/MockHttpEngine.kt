...
internal fun createMockHttpEngine(): HttpClientEngine = MockEngine { request ->
    ...
    when (request.method) {
        HttpMethod.Get -> when {
            ...
            path.startsWith(prefix = URL.CatastrophesTranslations.path) -> respondMock(content = encode(value = FakeData.catastrophesTranslations.get()))
            path.startsWith(prefix = URL.Engines.path) -> respondMock(content = encode(value = FakeData.engines.get()))
            path.startsWith(prefix = URL.EnginesTranslations.path) -> respondMock(content = encode(value = FakeData.enginesTranslations.get()))
            ...
            path.startsWith(prefix = URL.EventsTranslations.path) -> respondMock(content = encode(value = FakeData.eventsTranslations.get()))
            path.startsWith(prefix = URL.Achievements.path) -> respondMock(content = encode(value = FakeData.achievements.get()))
            path.startsWith(prefix = URL.AchievementsTranslations.path) -> respondMock(content = encode(value = FakeData.achievementsTranslations.get()))
